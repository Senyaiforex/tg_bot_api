...
class TextAdminData:
    ...
    text_pull: str = ('*Текущее состояние пулла:*\n\n'
                      'Пулл на монеты - _\r{coins}_\r\n'
                      'Пулл для фарминга - _\r{farming}_\r\n'
                      'Пулл для друзей - _\r{friends}_\r\n'
                      'Пулл для заданий - _\r{task}_\r\n'
                      'Пулл для краудсорсинга - _\r{plan}_\r\n')
    ...
    add_pull_invalid: str = "Пожалуйста, введите правильный пулл, он должен состоять только из цифр!"
    pull_input_success: str = ("Установлен новый пул на {key}.\n"
                               "Размер - {size}.\n"
                               "*Теперь введите пул на {next_key}*")
    pull_new_success: str = ("Пул на монеты - _\r{size}_\r\n"
                             "Фарминг - _\r{farming}_\r\n"
                             "Друзья - _\r{friends}_\r\n"
                             "Задания - _\r{task}_\r\n"
                             "Краудсорсинг - _\r{plan}_\r\n"
                             "Вы хотите установить новый пул с вышеуказанными значениями?")
    pull_empty: str = ("Пожалуйста, введите правильный пул для всех остальных"
                       " составляющих(монеты, друзья, краудсорсинг, фарминг. "
    ...
    text_liquid: str = ("*Пул ликвидности*\n\n"
                        "Постов бесплатно - {need_free}.На данный момент - {current_free}\n"
                        "Постов за монеты - {need_coins}.На данный момент - {current_coins}\n"
                        "Постов за токены - {need_token}.На данный момент - {current_token}\n"
                        "Постов за рубли - {need_money}.На данный момент - {current_money}\n"
                        "Постов за звёзды - {need_stars}.На данный момент - {current_stars}")
...
class TextUserData:
    ...
    name_product: str = ("Введите название товара\n\n"
                         "_\rдлина названия товара не должна превышать "
                         "75 символов и не должна иметь эмоджи-смайлов_\r")
    name_invalid: str = ("Длина названия "
                         "товара не должна превышать "
                         "70 и не должна иметь эмоджи-смайлов\n"
                         "Отправьте сообщение ещё раз или нажмите /start")
    ...
    negative = (f"✖️ В вашем листе ожидания уже имеется 10 товаров\n"
    ...
    save_photo = ("Фото товара _\r'{product_name}'_\r успешно сохранено! "
                  ...
                  f"Обратите внимание, стомость товара должна быть не меньше нуля и без пробелов и знаков препинания")
    ...
    discount_price = ("Укажите стоимость товара со скидкой(Кэшбеком).\n\n"
                      "_\rОбратите внимание, что скидка должна быть "
                      "реальная и отличаться от стоимости "
                      "на маркетплейсе минимум на 15%_\r\n"
                      "Введите стоимость без пробелов и знаков препинания")
    marketplace = "Выберите, на каком маркетплейсе будет покупка, либо нажмите на кнопку 'Пропустить'"

    url_acc = ("Укажите никнейм телеграм аккаунта, которому будут "
               "писать покупатели, переходящие по "
               "кнопке 'узнать условия'\n\n"
               "Никнейм аккаунта - Имя пользователя телеграмм аккаунта без знака '@'"
               )
    ...
    info_search = ("Здесь Вам доступны следующие действия:\n\n"
                   ...
                   "Обратите внимание, что максимальное количество товаров в листе ожидания - 10")
